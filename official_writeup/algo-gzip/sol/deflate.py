...
class Bitfield(BitfieldBase):
    ...
    def readbits(self, n=8):
        print("!!! readbits", n, self._xxtotal%8)
        self._xxtotal += n
        if n > self.bits:
            self.needbits(n)
...
class HuffmanTable:
    ...
    def find_next_symbol(self, field, reversed_=True):
        cached_length = -1
        cached = None
        for x in self.table:
            ...
            if (reversed_ and x.reverse_symbol == cached) or (not reversed_ and x.symbol == cached):
                ...
                print("found symbol", hex(cached), "at", field.tell(), "of len", cached_length, "mapping to", hex(x.code))
                return x.code
...
def bwt_transform(L):
    ...
    pointers = [-1] * len(L)
    for i, symbol in enumerate(map(ord, L)):
...
def to_bits_array(s: bytes) -> list[int]:
    return [int(x) for x in bin(int.from_bytes(s, 'little'))[2:].rjust(len(s) * 8, '0')[::-1]]

def judge_bit(item: HuffmanLength, target_bits: list[int]) -> bool:
    return item.bits <= len(target_bits) and item.symbol == int(''.join(map(str, target_bits[:item.bits])), 2)
...
def make_pad(alphabet: list[int], succ_target: int) -> bytes:
    import random
    random.seed(0)
    pad = b''
    while True:
        ...
        new_pad = pad + bytes(alphabet)
        if check_pad(new_pad):
            pad = new_pad
            succ_target -= 1
            if succ_target==0:
                break
...
def add_trail(ret: list[int], huff: OrderedHuffmanTable) -> bytes:
    from collections import Counter
    cntr = Counter(ret)
    max_cnt = cntr.most_common(1)[0][1]

    print('max cnt', max_cnt)

    trail = []
    for item in huff.table:
        if item.code<=127:
            for _ in range(max_cnt - cntr[item.code]):
                trail.append(item.code)

    #print('trail', trail)
    return bytes(ret) + make_pad(trail, 1)

def do_construct(leading_bits: int, huff: OrderedHuffmanTable) -> None:
    global RESULT_TARGET
    print('!do_construct')
    
    target_bits = [0]*(8-leading_bits) + to_bits_array(TARGET) + [0]*16

    print(f'leading bits {leading_bits}')
    print('target bits', target_bits)
    
    idx = 0
    ret = []
    while idx < len(target_bits)-16:
        for item in huff.table:
            if judge_bit(item, target_bits[idx:]):
                print(f'{idx}: found {item.code} with len {item.bits} bits {target_bits[idx:idx+item.bits]}')
                if item.code>127:
                    raise RuntimeError(f'bad code {item.code}')
                idx += item.bits
                ret.append(item.code)
                break
        else:
            print(f'{idx}: not found')
            raise RuntimeError('target not found in huffman')

    RESULT_TARGET = add_trail(ret, huff)

def do_construct_null(huff: OrderedHuffmanTable) -> None:
    global RESULT_NULL
    print('!do_construct_null')
    
    null_codes = []
    for item in huff.table:
        if bin(item.symbol).count('1')<=2:
            if item.code<256:
                null_codes.append(item.code)
    
    print('null_codes', len(null_codes))
    
    ret = list(make_pad(null_codes, 15))
        
    RESULT_NULL = add_trail(ret, huff)

# Sixteen bits of magic have been removed by the time we start decoding
def gzip_main(field, skip_header):
    b = Bitfield(field)
    if not skip_header:
        method = b.readbits(8)
        if method != 8:
    ...
    while True:
        ...
        print("deflate-blocktype", blocktype, ["stored", "static huff", "dyna huff"][blocktype], 'beginning at',
              header_start)
        ...
        if blocktype == 0:
            ...
            if blocktype == 1:  # Static Huffman
                ...
                #raise RuntimeError('!!! is static huffman')
            ...
            if blocktype == 2:
                do_construct(b.tell()[1], main_literals)
                do_construct_null(main_literals)
                return
            ...
            while True:
                ...
                if 0 <= r <= 255:
                    ...
                    if 0 <= r1 <= 29:
                        ...
                        print('copy', -distance, 'num bits', b.tellbits() - lz_start, 'data',
...
def decode(f):
    ...
    if magic == 0x1f8b:  # GZip
        out = gzip_main(field, False)
    # elif magic == 0x425a:  # BZip2
    #     out = bzip2_main(field)
    else:
        out = gzip_main(field, True)
