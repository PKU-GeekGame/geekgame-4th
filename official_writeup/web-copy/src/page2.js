...
// ----- XMCP: above are fernet

function embedCentralNoise(root, init_data) {
    const noiseContent1 = root.getElementById('centralNoiseContent1');
    decryptMain(noiseContent1, init_data);
}
...
function anti_hacker() {
    //alert('!!!hacker');
    /////XMCP-TODO
    document.documentElement.innerHTML = '<h1>有黑客！</h1>';
    window.location.href = '/hacker';
}

// XMCP: below is github.com/david-fong/detect-devtools-via-debugger-heartstop
...
onmessage = (ev) => { postMessage({hackr:true});
	...
	const doOnePulse = () => {
		heart.postMessage({ hackr: config.moreAnnoyingDebuggerStatements });
	}
...
// XMCP: above is github.com/david-fong/detect-devtools-via-debugger-heartstop

// XMCP: below is stackoverflow.com/a/41220267/5210803

let runBotDetection = function () {
...
// XMCP: above is stackoverflow.com/a/41220267/5210803
...
const XOR_KEY = [780777,1604188,388112,1562985,728065,127585,3450,206950,226564,46185,68976,54353,140649,1063249,12817,97905,1126569,1398,1330033,127585,420790,1296628,420790,857482,68976,206950,1562985,2769,300238,593769,2729,113321,1562985,613564,186176,2721,54353,1398,11298,50636,1953092,344017,18177,7017,46185,752646,1093170,941857,3450,2386,226564,5345,1257,15776,780777,1330033,357522,12817,1191489,127585,65705,206950,1482593,1001577,277224,186176,1644689,388112,514729,1604188,1985,140649,68976,1815937,1861030,92084,804004,2604,118808,12817,4337,613564,1330033,4337,887465,41505,1644689,35497,30282,35497,103962,6008,41774,1032972,21993,168386,127585,573982,857482,226564,941857,140649,277224,912776,149020,149020,113321,912776,474609,357522,92084,2729,553441,1443416,613564,1905641,1001577,314985,175273,474609,28977,1262313,1985,277224,2256,1407657,168386,2702,553441,553441,5345,514729,659042,1093170,175273,374953,226564];

function xorDecrypt(data, key) {
    let decrypted = '';
    const keyLength = key.length;
    for (let i = 0; i < data.length; i++) {
        decrypted += String.fromCharCode(data.charCodeAt(i) ^ Math.round(Math.cbrt((key[keyLength - 1 - i % keyLength]-810) ^ 1919)));
    }
    return decrypted;
}
...
function makeChunks(ss, n) {
    let s = [...ss];
    let ret = [];
    let chunksize = Math.ceil(s.length/n);
    for(let i=0; i<n; i++)
        ret.push(s.slice(i*chunksize, (i+1)*chunksize).join(''));
    return ret;
}
...
function decryptMain(dom, data) {
    let secret = ['VRmLgWs4ivtpwguMqoM_egcU01KEuwitL5TU=', String.fromCharCode(107, 86, 78, 55, 107, 75)].reverse().join('-');

    let html = '';
    let style = [];
    style.push(`.chunk{font-size:0;color:transparent}`);
    style.push(`.chunk::before,.chunk::after{font-size:1rem;color:rgba(0, 255, 0, 0.6)}`);

    data.split('|').forEach((chunk)=>{
        let text = new window.pLgzf.Token({
            secret: new window.pLgzf.Secret(secret),
            token: chunk,
            ttl: 0
        }).decode();
        text = xorDecrypt(text, XOR_KEY);
        
        for(let tt of makeChunks(text, 4)) {
            let [tt_before, tt_after] = makeChunks(tt, 2);
            let serial = randomId();
            let attr = [];
            let content_before = [];
            let content_after = [];
            for(let t of makeChunks(tt_before, 4)) {
                let id = randomId();
                attr.push(`data-${id}="${t}"`);
                content_before.push(`attr(data-${id})`);
            }
            for(let t of makeChunks(tt_after, 4)) {
                let id = randomId();
                attr.push(`data-${id}="${t}"`);
                content_after.push(`attr(data-${id})`);
            }
            shuffleArray(attr);
            html += `<span class="chunk" id="chunk-${serial}" ${attr.join(' ')}>兄弟你好香</span>`;
            style.push(`#chunk-${serial}::before{content:${content_before.join(' ')}}`);
            style.push(`#chunk-${serial}::after{content:${content_after.join(' ')}}`);
        }
        shuffleArray(style);
    });
    ...
    styleElem.textContent = style.join('');
    dom.parentNode.appendChild(styleElem);
}
...
function generateFloatingElements(root) {
    const container = root.getElementById('floatingElementsContainer');
...
function main() {
    const container_elem = document.getElementById('root');
    const init_data = container_elem.textContent;
    container_elem.textContent = '';
    container_elem.style.display = 'initial';

    const root = container_elem.attachShadow({ mode: 'closed' });
    ...
    * {
    ...
    [1, 2, 3].forEach(()=>generateFloatingElements(root));
    embedCentralNoise(root, init_data);
    setInterval(()=>generateFloatingElements(root), 8000);
}
