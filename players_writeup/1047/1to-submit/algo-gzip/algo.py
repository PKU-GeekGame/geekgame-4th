...
def equalize_token_counts(crit_tokens: list[int], path_length: int, length_limit: int, token2ignore) -> list[int]:
    token_num_limit = length_limit
    each_token_num_limit = token_num_limit // ((1 << path_length)-1)
...
small_bitcnt_nums = []
bitcnt_3_nums = []
for x in range(0, 1<<PATH_LENGTH):
    cnt = 0
    for i in range(0, 8):
        cnt += (x>>i)&1
    if cnt <= 2:
        small_bitcnt_nums.append(x)
    if cnt == 3:
        bitcnt_3_nums.append(x)
print(f"{len(small_bitcnt_nums)=}, {len(bitcnt_3_nums)=}")

# crit_tokens = [random.choice(small_bitcnt_nums) for _ in range(340)]
crit_tokens = []
for token in small_bitcnt_nums:
    crit_tokens.extend([token]*15)
random.shuffle(crit_tokens)
crit_tokens = crit_tokens[:300]
print("crit tokens:", crit_tokens)
print("crit token length after encoding:", len(crit_tokens)*PATH_LENGTH/8)
...
tokens = equalize_token_counts(crit_tokens, PATH_LENGTH, 1000, nonused_token)
# print(tokens)
