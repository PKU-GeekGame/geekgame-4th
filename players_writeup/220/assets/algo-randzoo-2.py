# Symbolic Merseene Cracker: https://book.jorianwoltjer.com/cryptography/pseudo-random-number-generators-prng#truncated-samples-symbolic-solver
...
import string
POSSIBLE_ORDS = [ord(c) for c in string.printable if c not in string.whitespace]

if __name__ == '__main__':
    cracker = Untwister()
    with open("./randzoo-nums.txt", "r") as f:
        for i in range(2000):
            unknown = int(f.readline())
            pattern = [bit for bit in bin(unknown)[2:].zfill(32)]
            for o in POSSIBLE_ORDS:
                for r, pattern_bit, o_bit in zip(range(32), pattern[:], bin(unknown - o)[2:].zfill(32)):
                    if pattern_bit == '?':
                        continue
                    if pattern_bit != o_bit:
                        pattern[r] = '?'
            cracker.submit("".join(pattern))
            print(f"{i}, {sum(1 for bit in pattern if bit != '?')}")
        rnd = cracker.get_random()
        for _ in range(500):
            print(chr(int(f.readline()) - rnd.getrandbits(32)), end="")
